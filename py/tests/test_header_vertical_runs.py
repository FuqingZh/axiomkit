...
from axiomkit.io.xlsx_formatter import (  # noqa: E402
    BorderSpec,
    _iter_vertical_runs,
    _plan_vertical_visual_merge_borders,
    _remove_vertical_run_text,
)


def test_iter_vertical_runs_empty_grid_yields_nothing() -> None:
    assert list(_iter_vertical_runs([])) == []


def test_iter_vertical_runs_single_row_yields_nothing() -> None:
    grid = [["A", "B", "C"]]
    assert list(_iter_vertical_runs(grid)) == []
...
def test_plan_vertical_visual_merge_borders_sets_top_bottom_borders_per_run() -> None:
    ...
    plan = _plan_vertical_visual_merge_borders(grid)

    assert plan[(0, 0)] == BorderSpec(top=1, bottom=0, left=1, right=1)
    assert plan[(1, 0)] == BorderSpec(top=0, bottom=0, left=1, right=1)
    assert plan[(2, 0)] == BorderSpec(top=0, bottom=1, left=1, right=1)

    assert plan[(4, 0)] == BorderSpec(top=1, bottom=0, left=1, right=1)
    assert plan[(5, 0)] == BorderSpec(top=0, bottom=1, left=1, right=1)
...
def test_remove_vertical_run_text_blanks_non_top_cells_in_place() -> None:
    ...
    returned = _remove_vertical_run_text(grid)
